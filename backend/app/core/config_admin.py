...
CONFIG_FIELDS: list[ConfigFieldMeta] = [
    ...
    ConfigFieldMeta("CRAG_RELEVANCE_THRESHOLD", "rag", "CRAG 相关性阈值", "Agentic RAG 中 CRAG 检索质量评估的平均 rerank_score 阈值，低于该值时触发一次纠错重试。", parse_as="float", input_type="number", default=2.0),
    ConfigFieldMeta("SELF_RAG_ENABLED", "rag", "启用 Self-RAG 后验检查", "开启后 Agentic RAG 会在生成答案后进行一次自我检查，必要时保守修正回答。", parse_as="bool", input_type="switch", default=False),
    ConfigFieldMeta("WEB_SEARCH_ENABLED", "rag", "启用 Web Search 兜底", "开启后仅在 Agentic RAG 的 CRAG 重试后仍低质时调用外部网页搜索作为补充上下文。", parse_as="bool", input_type="switch", default=False),
    ConfigFieldMeta("BAIDU_WEB_SEARCH_API_KEY", "rag", "百度 Web Search API Key", "百度千帆智能搜索生成接口的 API Key，通过 X-Appbuilder-Authorization 请求头发送。", sensitive=True, input_type="password"),
    ConfigFieldMeta("BAIDU_WEB_SEARCH_ENDPOINT", "rag", "百度 Web Search 地址", "百度千帆智能搜索生成 web_summary 接口地址。", parse_as="url", default="https://qianfan.baidubce.com/v2/ai_search/web_summary"),
    ConfigFieldMeta("BAIDU_WEB_SEARCH_TOP_K", "rag", "百度 Web Search 引用数", "百度网页搜索最多返回并展示的网页引用数量。", parse_as="int", input_type="number", default=5),
    ConfigFieldMeta("BAIDU_WEB_SEARCH_TIMEOUT_SECONDS", "rag", "百度 Web Search 超时", "百度网页搜索请求的最长等待时间，单位秒。", parse_as="int", input_type="number", default=30),
